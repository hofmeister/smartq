...
public class SocketProxy {
    ...
    public SocketProxy(InetSocketAddress address, InetSocketAddress target) throws Exception {
    ...
    public void open() throws Exception {
        ...
        serverSocket.bind(address, acceptBacklog);
    ...
    public void reopen() {
        ...
        try {
            ...
            LOG.debug("exception on reopen url:" + address, e);
        }
    }
    ...
    public class Bridge {
        ...
        public Bridge(Socket socket, InetSocketAddress target) throws Exception {
            receiveSocket = socket;
            sendSocket = createSocket();
            if (receiveBufferSize > 0) {
                sendSocket.setReceiveBufferSize(receiveBufferSize);
            }
            sendSocket.connect(target);
    ...
    public class Acceptor implements Runnable {
        ...
        public Acceptor(ServerSocket serverSocket, InetSocketAddress target) {
            socket = serverSocket;
            this.target = target;
            pause.set(new CountDownLatch(0));
            try {
